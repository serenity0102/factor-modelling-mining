...
class BaseFactor:
        ...
        if 'factor_test_results' in self.results and not self.results['factor_test_results'].empty:
            try:
                ...
                if 'T-stat' in self.results['factor_test_results'].columns:
                    plt.subplot(2, 2, 2)
                    sns.histplot(self.results['factor_test_results']['T-stat'], kde=True)
                ...
                print(f"Error plotting factor test results: {str(e)}")
                plt.close('all')
    ...
    def analyze_test_factor(self, returns_df, portfolio_returns, output_dir):
    ...
    def analyze_evaluate_portfolio(self, factor_test_results, portfolio_returns, output_dir):
        ...
        try:
            ...
            # Remove any rows with factor_name if present
            if 'factor_name' in performance_results.index:
                performance_results = performance_results.drop('factor_name')
