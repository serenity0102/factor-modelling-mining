...
class BaseFactor:
    ...
    def plot_results(self, save_dir='.'):
        ...
        plt.subplot(2, 2, 4)
        rolling_factor = self.results['portfolio_returns'][f'{self.name}_Factor'].rolling(window=min(252, len(self.results['portfolio_returns']))).mean()
        rolling_factor.plot()
        plt.title(f'{min(252, len(self.results["portfolio_returns"]))}-Day Rolling Average of {self.name} Factor Returns')
